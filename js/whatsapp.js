...
function generateBookingMessage(bookingData) {
  ...
  // Service-specific details block (only when the service provided extra fields)
  const detailsBlock = (serviceDetails && serviceDetails.length)
    ? `\n\n*${service} Details:*\n${serviceDetails.join('\n')}`
    : '';

  return `*NEW BOOKING REQUEST*\n\n*Service:* ${service}\n*Package:* ${packageName}\n*Price:* ${price}\n\n*Customer Details:*\nName: ${name}\nWhatsApp: ${whatsapp}\nEmail: ${email}\n\n*Event Details:*\nDate: ${eventDate}\nTime: ${eventTime}\nLocation: ${location}${detailsBlock}\n\n*Special Requests:*\n${message || 'None'}`;
}
...
function handleBookingSubmit(event) {
  ...
  const bookingData = {
    ...
    // Collect any service-specific fields injected by booking.js
    serviceDetails: Array.from(form.querySelectorAll('.js-service-field'))
